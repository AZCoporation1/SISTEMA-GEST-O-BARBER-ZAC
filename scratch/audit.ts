...
async function runAudit() {
  ...
  try {
    ...
    // Fallback if execute_sql is not available (it's a custom RPC usually)
    // We will query pg_proc to find functions if we can't query triggers directly
    const { data: functions, error: funcError } = await supabase.rpc('execute_sql', {
      query_text: `
        ...
        where pg_get_functiondef(p.oid) ilike '%user_profiles%'
           or pg_get_functiondef(p.oid) ilike '%handle_new_user%';
      `
    })

    if (triggerError) {
      console.log("-> Não foi possível executar query direta de triggers (RPC execute_sql ausente).")
      console.log("-> Tentando buscar a função 'handle_new_user' via REST normal se existir...")
    } else {
